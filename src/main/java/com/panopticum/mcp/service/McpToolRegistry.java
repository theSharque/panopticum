        ...
        return switch (toolName) {
            ...
            case "query-data" ->
                "Execute a query and return a unified JSON envelope. Required: connectionId (number), query (string). Optional: catalog, namespace, entity, offset, limit (hard max 100). " +
                        "SQL: SQL text. CQL: CQL text. N1QL: N1QL text (Couchbase). Mongo/Elasticsearch: JSON (MQL/DSL). Kafka: JSON {\"partition\",\"fromOffset\",\"count\",\"fromEnd\"}, catalog=topic. Redis: glob pattern (e.g. user:*), catalog=dbIndex. S3: JSON {\"headBytes\",\"format\"}, catalog=bucket, entity=key. Prometheus: PromQL or JSON range query. RabbitMQ: peek — count or JSON {\"count\"}; publish — JSON {\"publish\":[\"msg\",{\"k\":\"v\"}]}, catalog=vhost, entity=queue.";
